...
proto.execute = function(sql, params, options, callback) {
  ...
  if (options.fetchRows && !options.cursor)
    pgQuery.rows = options.fetchRows;

  waterfall([
    /* Start transaction if autoCommit=false */
    function(next) {
      if (options.autoCommit || self._inTransaction)
        return next();
      self.intlcon.query('BEGIN', function(err) {
        if (!err)
    ...
    /* Commit transaction if autoCommit=true */
    function(next, result) {
      if (!options.autoCommit)
        return next(undefined, result);
      self.commit('COMMIT', function(err) {
        next(err, result);
      });
    }
  ], callback);
...
proto.commit = function(callback) {
  this.intlcon.query('COMMIT', function(err) {
    if (!err)
...
proto.rollback = function(callback) {
  this.intlcon.query('ROLLBACK', function(err) {
    if (!err)
      self._inTransaction = false;
    callback(err);
  });
...
proto.test = function(callback) {
  const self = this;
  self.intlcon.execute('select 1', [], function(err) {
    return callback(err);
  });
};
